...
public class AerospikeTemplate implements AerospikeOperations {
	...
	/**
	 * add via the key is byte[] query
	 * @param ids
	 * @param type
	 * @param <T>
	 * @return
	 */
	public <T> List<T> findByIds(List<byte[]> ids, Class<T> type) {
		...
		try {
			...
			for (int i = 0; i < ids.size(); i++) {
				keys[i] = getGenderKey(ids.get(i), entity);
			}
	...
	private Key getGenderKey(byte[] id, AerospikePersistentEntity<?> entity) {
		return new Key(this.namespace, entity.getSetName(), id);
	}
